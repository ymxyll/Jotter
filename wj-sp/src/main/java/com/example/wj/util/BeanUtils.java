...
//功能：
//
//1. transformFrom：将源对象的属性值复制到目标类型的实例中（仅复制相同的属性），并返回该实例。如果源对象为 null，则返回 null。
//2. transformFromInBatch：批量将源集合中的元素映射为目标类型的实例，并返回目标实例构成的集合。
//3. updateProperties：将源对象的非空属性值复制到目标对象中。
//4. getNullPropertyNameSet：获取源对象中所有属性值为null的属性名构成的集合。
